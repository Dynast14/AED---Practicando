...
int main(){
    ...
    hash_table<int,bool> conjuntoDeVistos;
    int contadorDeDistintos = 0;

    for(int indice = 0; indice < cantidadDeElementos; indice = indice + 1){
        int numeroActual;
        cin >> numeroActual;

        bool yaLoHabiamosVisto = conjuntoDeVistos.contains(numeroActual);

        if(yaLoHabiamosVisto == false){
            conjuntoDeVistos[numeroActual] = true;
            contadorDeDistintos = contadorDeDistintos + 1;
        }
    }

    cout << contadorDeDistintos << "\n";
